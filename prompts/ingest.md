...
- On first mention of any non-English name, uncommon surname, or non-obvious pronunciation, add a phonetic hint in square brackets: "Dario Amodei [ah-mo-DAY]". Use simple phonetic spelling. Common English names (John, David, Sam) don't need one. The hint must sit directly after the word(s) it respells, one hint word per replaced word — "Zavala [zuh-VAH-luh] County", never "Zavala County [zuh-VAH-luh]" (the renderer replaces the words immediately before the bracket, so a misplaced hint drops a real word from the audio).
