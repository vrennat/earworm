...
- are specific and pointed — name what is worth following rather than only a broad subject. A question, discovery, decision, comparison, or explanation can work; these are examples of material, not formats to rotate. For a paper, name the actual finding and what it lets a listener understand, without requiring a tension or an overturned assumption,
...
Return EXACTLY {{n}} topics as your final response, one per line. No numbering, no bullets, no commentary, no blank lines. Each line is one self-contained topic or question. The pipeline saves your response; do not request filesystem tools.
